...
const getLearningStateStorageKey = (email = studentData.value.email) => email ? `$mds_gms2b_learning_state:${String(email).toLowerCase().trim()}` : 'mds_gms2b_learning_state';
const getProgressStorageKey = (email = studentData.value.email) => email ? `mds_gms2b_student_progress:${String(email).toLowerCase().trim()}` : 'mds_gms2b_student_progress';
const getLoginStorageKey = () => 'mds_gms2b_student_login';
...
const submitProject = async () => {
  if (projectUploadType.value === 'link') {
    ...
    reader.onload = async (e) => {
      ...
      const fileNameStr = `${studentData.value.name}_${studentData.value.school}_grupB_${projName}_${projectFile.value.name}`;
      const fileCol = 'Project1_Code';
...
const persistLearningState = ({ force = false } = {}) => {
  ...
  localStorage.setItem(getLearningStateStorageKey(), JSON.stringify({
        ...
        <section class="step-panel" v-for="(data, step) in courseData" :key="step" :id="'step-' + step" v-show="currentStep === parseInt(step)">
          ...
          <div class="reading-container" v-if="data.summaryHtml">
            <details class="lesson-reading-accordion" :open="isDesktop ? true : undefined">
              ...
              <div v-if="currentStep === 5" class="project-upload-area" style="background: var(--paper); border-radius: 16px; padding: 32px; margin-top: 32px; border: 2px dashed var(--line); text-align: center; box-shadow: var(--shadow);">
